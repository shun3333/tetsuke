...
/**
 * 手組内の掛け声1つ。
 * rel_pos は手組の頭(= 置いたクサリの1拍目の表)からの相対位置を
 * 「半拍単位」で表す。
 *   0 = 1拍の表 / 1 = 1拍の裏 / 2 = 2拍の表 / …
 * 負の値は手組の頭より前を指す。大鼓を中心に、前のクサリの最後の拍
 * (本地なら8拍目)から始まる手組が多いため、そこを次のように表す。
 *   -2 = 前のクサリの最後の拍の表 / -1 = その裏
 ...
 *
 * 起点はクサリの1拍目(表)に固定で、選べるのは「どのクサリに置くか」だけ。
 * 手組がクサリの拍数より長い場合、続きは自動的に次のクサリに乗る。
 * 逆に、負の rel_pos を持つ手組は前のクサリにはみ出す
 * (手組はクサリをまたいでよい)。
 */
