 ...
 *
 * circle / small / height_scale / spacing / dx / dy は見た目の微調整で、
 * どれも省いてよい。
...
export interface ShogaChar {
  ...
  /**
   * 文字の代わりに○を書く。
   * text はそのまま持っておくので、外せば元の文字に戻る。
   * ○だけを置きたい枠は text が空のままでよい。
   */
  circle?: boolean;
  /** 拗音のように、1回り小さく書く */
  small?: boolean;
 ...
 *
 * 起点はクサリの1拍目から1拍前にずらした位置(= 前のクサリの
 * 最後の拍の表)に固定で、選べるのは「どのクサリに置くか」だけ。
